...
HIGH_LEVEL_MODEL_PATH = "models/hl_improved_2M/best_model.zip"

print("Starting High-Level Navigation Evaluation and creating environment...")

# Create environment with CORRECT low-level model
env = HRLHighLevelEnv(low_level_model_path=LOW_LEVEL_MODEL_PATH,subgoal_distance=6.0,option_horizon=40,debug=True)
# Load hig-level policy
...
for ep in range(n_episodes):
    ...
    print(f"\nEPISODE {ep + 1}/{n_episodes},Start Position : {agent_start},Main Goal : {main_goal}")
    print(f"Start Distance : {start_distance:.2f}m\n")
    episode_done = False
    step_count = 0
    for step in range(20):
        step_count += 1        
        # Predict high-level action
        ...
        print(f"Distance to goal: {info['main_distance']:.2f}m,Reward: {reward:.3f}")
        if done:
